...
public class MarketPlaceRegressionRunner {

    @BeforeClass
    public static void beforeClass() {
        System.out.println("Initializing RegressionRunner to run test");
        List<String> tags = Arrays.asList("@securityToken");
        List<String> features = Arrays.asList("src/test/features/ApiMarketPlace/");
    ...
    public void regressionTest() {
        try{
            List<String> tags = Arrays.asList("~@securityToken","~@notIncluded");
            List<String> features = Arrays.asList("src/test/features/ApiMarketPlace/");
            String karateOutputPath = "target/surefire-reports";
            KarateStats stats = CucumberRunner.parallel(tags, features, 1, karateOutputPath);
