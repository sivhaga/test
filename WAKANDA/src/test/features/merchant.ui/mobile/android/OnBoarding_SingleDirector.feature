...
#    Then MA user is on sign up page
    ...
    And MA user enter house number and complex name "<house number and complex name>"
    ...
    And MA user select next button
    Then MA user is on scan your ID page
    And MA user select continue button
    # step missing
    Then MA user is on scan your ID page
    Then MA user can see document uploaded label
    When MA user select continue button
    Then MA user is on business details page
    When MA user enter business registration number "<business registration number>"
    And MA user enter business registered name "<business registered name>"
    And MA user enter trading name "<trading name>"
    And MA user select business type "<business type>" from the list
    And MA user enter date of incorporation "<date of incorporation>"
    And MA user enter business phone number "<business phone number>"
    And MA user select financial year end "<financial year end>" from the list
    And MA user select industry "<financial year end>" from the list
    And MA user select business turnover "<financial year end>" from the list
    And MA user select source of funds "<financial year end>" from the list
    And MA user select yes to have business tax obligations
    And MA user can see Nedbank's obligations displayed
    And MA user select save and continue button
    Then MA user is on business address page
    And MA user enter house number and complex name "<house number and complex name>"
    And MA user enter street name "<street name>"
    And MA user enter suburb "<suburb>"
    And MA user enter city "<city>"
    And MA user enter postal code "<postal code>"
    And MA user select that postal address same as trading address
    And MA user select continue button
    Then MA user is on company details page
    When MA user upload business address
    And MA user upload CIPC business registration documents
    And MA user select submit button
    Then MA user is on congratulations page
    Then MA user can see message business wallet will be activated within 72 hours and they will be notified once activated
    When MA user select continue button
    And MA user select first subcategory
    And MA user select submit button
    And MA user select second subcategory
    ...
    Examples:
      | south african id | cell phone number |  occupation | otp | username   | password   | confirm password | pin | confirm pin |trading name|house number and complex name|street name|suburb|city|postal code|
      | 9111225184080    | 0617373770        |             |     | Testuser10 | Testing@01 | Testing@01       |     |             |            |                             |           |      |    |           |
...
#    Then MA user is on sign up page
    ...
    Examples:
      | south african id | cell phone number | first name | last name | otp | username | password   | confirm password | pin | confirm pin |trading name|house number and complex name|street name|suburb|city|postal code|
...
#    Then MA user is on sign up page
    ...
    Examples:
      | south african id | cell phone number | otp | username | password | confirm password | pin | confirm pin |trading name|house number and complex name|street name|suburb|city|postal code|
      |                  |                   |     |          |          |                  |     |             |            |                             |           |      |    |           |
...
#    Then MA user is on sign up page
    ...
    When MA user set location
    And MA user select submit button
    Then MA user is on dashboard pagek
